...
public class CSRenderer extends JPanel {
  ...
  public void paintComponent(Graphics g) {
    ...
    for (CSPoint point : cs.getAllPoints()) {
      ...
      g2d.fillOval(translatedPoint.x - pointSize / 2, translatedPoint.y - pointSize / 2, pointSize, pointSize);
    }

    // draw line segments
    g2d.setStroke(new BasicStroke(fieldScale));
    g2d.setColor(Color.GREEN);
    for (CSLineSegment segment : cs.getAllLineSegments()) {
      CSPoint translatedStart = translatePoint(segment.getStartPoint());
      CSPoint translatedEnd = translatePoint(segment.getEndPoint());
